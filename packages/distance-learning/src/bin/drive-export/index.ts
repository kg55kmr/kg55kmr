...
const progress = (size: string = "") =>
  process.stdout.write(`${count}: ${size}         \r`);
...
for (const url of urls) {
  ...
  if (resp.ok && resp.body) {
    ...
    // @ts-expect-error because fetch is web fetch, not a node one
...
async function get(link: string) {
  ...
  if (name === undefined) throw new Error(`${url} cannot get name`);
...
async function checkIfBigFile(url: string) {
  const resp = await fetch(url);
